...
module.exports = {
	...
	playTrack: function(tid) {
		return {type:C.PLAY_TRACK, tid};
	},
	startQuoteEdit: function(qid) {
		return {type:C.START_TRACK_EDIT, qid};
	},
	cancelQuoteEdit: function(qid) {
		return {type:C.FINISH_TRACK_EDIT, qid};
	},
	deleteQuote: function(qid) {
		return function(dispatch, getState) {
			dispatch({type:C.SUBMIT_TRACK_EDIT, qid});
			tracksRef.child(qid).remove(function(error) {
				dispatch({type:C.FINISH_TRACK_EDIT, qid});
				if (error) {
	...
	submitNewQuote: function(content) {
		return function(dispatch, getState) {
			...
			if (error) {
				...
				dispatch({type:C.AWAIT_NEW_TRACK_RESPONSE});
				tracksRef.push({content,username,uid},function(error) {
					dispatch({type:C.RECEIVE_NEW_TRACK_RESPONSE});
					if (error) {
